...
const AddClusters = (props: { refetch: any, handleAddClusters: any }) => {
  ...
  const handleAddCluster = async (): Promise<void> => {
    try {
      const body = {
        ...
        cost_Url: (
        document.getElementById('cost_Url') as HTMLInputElement
        ).value,
        cost_port: (
          document.getElementById('cost_port') as HTMLInputElement
          ).value,
        
      };
      const arr :string[]= [body.url,body.k8_port,body.faas_port,body.name,body.description,body.faas_url,body.grafana_url, body.kubeview_url ]
      if (
        !body.url ||
        !body.k8_port ||
        !body.faas_port ||
  ...
  const handleChange = (event: React.SyntheticEvent, newValue: number) => {
    ...
    return (
        ...
        //   </Box>
        <>
         <Box id='tenOptions' sx={{ backgroundColor: 'grey',
                color: 'black',
